...
func newExpenseSearchCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github.com/mvanhorn/printing-press-library/library/productivity/expensify expense search "coffee"
  github.com/mvanhorn/printing-press-library/library/productivity/expensify expense search "uber" --since 2026-01-01 --limit 50`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			w := cmd.OutOrStdout()
			if len(results) == 0 {
				fmt.Fprintf(w, "No matches for %q (run `github.com/mvanhorn/printing-press-library/library/productivity/expensify sync` if the local store is empty).\n", query)
				return nil
			}
