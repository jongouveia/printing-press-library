...
func newExpenseDeleteCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  github.com/mvanhorn/printing-press-library/library/productivity/expensify expense delete --transaction-id <id>\n" +
			"  github.com/mvanhorn/printing-press-library/library/productivity/expensify expense delete --transaction-id <id> --report-id <rid> --report-action-id <aid>",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if !flags.dryRun && bodyTransactionID != "" && (bodyReportID == "" || bodyReportActionID == "") {
					rid, aid, rerr := resolveExpenseDeleteRefs(c, bodyTransactionID)
					if rerr != nil {
...
func resolveExpenseDeleteRefs(c apiPoster, transactionID string) (reportID, reportActionID string, err error) {
	// 1) transaction -> reportID
	txData, _, terr := c.Post("/Get", map[string]any{
	...
	rData, _, rerr := c.Post("/OpenReport", map[string]any{"reportID": reportID})
	if rerr != nil {
		return "", "", rerr
	}
