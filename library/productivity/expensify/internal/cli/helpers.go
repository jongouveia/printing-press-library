...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && looksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'github.com/mvanhorn/printing-press-library/library/productivity/expensify doctor' to check auth status."+
			"\n      Response: "+sanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 407") || strings.Contains(msg, "session expired"):
		return authErr(fmt.Errorf("%w\nhint: your Expensify session has expired. Re-authenticate with:"+
			"\n      github.com/mvanhorn/printing-press-library/library/productivity/expensify auth login --from-chrome   (reads the live token from Chrome)"+
			"\n      or: pbpaste | github.com/mvanhorn/printing-press-library/library/productivity/expensify auth set-token -"+
			"\n      Run 'github.com/mvanhorn/printing-press-library/library/productivity/expensify doctor' to check auth status.", err))
...
// paginatedGet removed: Expensify's dispatcher API returns full datasets per
// call and does not use cursor/offset pagination.
...
// extractResponseData removed: Expensify responses use an onyxData/jsonCode
// envelope but the display helpers walk that envelope inline where relevant.
